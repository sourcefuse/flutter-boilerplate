...
import 'package:clean_arch/presenter/ui/home/adapter/note_adapter.dart';
...
class NoteListBloc extends Bloc<NoteListEvent, NoteListState> {
  ...
  Future<void> getNoteList(
    ...
    try {
      ...
      if (result.success ?? false) {
        ...
        List<NoteViewModel> notes = _getNoteViewModelData();
  ...
  List<NoteViewModel> _getNoteViewModelData() {
    List<NoteViewModel> list = [];
    for (var item in noteList) {
      list.add(NoteAdapter.getNoteModelData(item));
    }
    return list;
  }
  ...
  Future<void> deleteNote(
    ...
    try {
      ...
      if (result.success ?? false) {
        final index = noteList
            .indexWhere((element) => element.uid == event.noteRequestModel.uid);
        listKey.currentState?.removeItem(
          index,
          (_, animation) {
            return _slide(animation,
                NoteTile(note: NoteAdapter.getNoteModelData(noteList[index])));
          },
